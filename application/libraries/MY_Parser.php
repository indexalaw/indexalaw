 ...
 * CI smartylib
 *
 * smartylib templating for Codeigniter
 *
 * @category CodeIgniter
 * @package  CI_smartylib
...
class MY_Parser extends CI_Parser
{
    ...
    public function __construct()
    {
        ...
        //$this->CI->load->library('smartylib');
    ...
    /**
     * Call
     * able to call native smartylib methods
     *
    ...
    public function __call($method, $params=array())
    {
        if (!method_exists($this, $method)) {
            return call_user_func_array(array($this->CI->smartyliblib, $method), $params);
        }
    }
    /**
     * Set Theme
     ...
     * Parse
     *
     * Parses a template using smartylib 3 engine
     *
        ...
        */
        ...
        if ( ! empty($data)) {
            foreach ($data AS $key => $val) {
                $this->CI->smartylib->assign($key, $val);
            }
        }
        // Load our template into our string for judgement
        $template_string = $this->CI->smartylib->fetch($template);
     ...
     * to smartylib so we can extend and include view files
    ...
    protected function _add_paths()
    {
        // Iterate over our saved locations and find the file
        foreach ($this->_template_locations AS $location) {
            $this->CI->smartylib->addTemplateDir($location);
        }
    }
    /**
     * Update Theme Paths
    ...
    protected function _update_theme_paths()
    {
        ...
        // Will add paths into smartylib for "smarter" inheritance and inclusion
        $this->_add_paths();
    }
    /**
     * String Parse
     *
     * Parses a string using smartylib 3
     *
    ...
    {
        return $this->CI->smartylib->fetch('string:'.$template, $data);
    }
    /**
     * Parse String
     *
     * Parses a string using smartylib 3. Never understood why there
